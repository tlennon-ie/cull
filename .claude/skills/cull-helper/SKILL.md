...
3. **Multi-folder local imports.** `scrapers.local_imports` is a **list** of `{name, dir, enabled, migrate_from}`; `resolve_env` emits enabled ones as `LOCAL_IMPORTS_JSON` and the supervisor fans them into per-agent `LOCAL_IMPORT_*` envs. Local folders are managed only through this list — don't add a dedicated local-folder entry to `SCRAPER_NAMES`.
