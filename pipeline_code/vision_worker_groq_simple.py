#!/usr/bin/env python3
"""
VISION WORKER - GROQ ONLY
Simple, reliable, hierarchical classification
No LMStudio complications
"""
...
def classify_with_groq(b64_image):
    ...
    try:
        ...
        prompt = """Analyze this image. Respond ONLY with valid JSON (no markdown, no explanation):
{
...
- style: What you see (selfie=mirror/phone POV, portrait=headshot, editorial=studio, cinematic=dramatic scene, vintage=retro aesthetic, fantasy=stylized/otherworldly, sports=athletic context, fashion=clothing-focused)."""
...
def classify_image(img_bytes, source_name):
    ...
    if result is None:
        return None, "DISCARD", "API failed"
    ...
    # InstagramInfluencer (selfie/portrait, good quality)
...
def reclassify_all():
    ...
    print("=" * 70)
    print("RECLASSIFYING ALL FILES - GROQ ONLY")
    print("=" * 70)
    print("")
    ...
    stats = {"processed": 0, "errors": 0, "reclassified": 0, "unchanged": 0}
    start = time.time()
    ...
    for i, img_path in enumerate(all_images):
        if i % 50 == 0:
            ...
            print(f"[{i}/{len(all_images)}] ETA: {remaining:.0f}s", flush=True)
        ...
        try:
            ...
            result, new_cat, reason = classify_image(img_bytes, correct_source)
